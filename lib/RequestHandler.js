 ...
 * @param {http.Agent} response - Send Response to client
 */
...
/**
 * Used to upload a specofoc KSOM
 * @param {*} response 
 * @param {*} postData 
 */
...
function replaceIngredeint(response, postData) {
    ...
    nosqldb.selectedIngredeint(postData.fields, function (err, result) {
        checkError(err);
...
function checkError(err) {
    if (err) {
        ...
        response.end("An error occured while processing your request. " + err);
    }
}
...
function findRecipeRange(response, postData) {
    nosqldb.queryIngredientInRange(postData.fields, (err, result) => {
        data = { documents: result, startTime: postData.time, count: result.length };
...
function deleteRecipeWF(response, postData) {
    nosqldb.queryDeleteWF(postData.fields, function (err, result) {
        sendResponse(err, data); data = { documents: result, startTime: postData.time, count: result.length };
        result.forEach((document) => nosqldb.deleteWF(document, (err, result) => checkError(err)));
    });
}
...
/**
 * Used to upload entire data directory into Azure Cosmos DB in JSON format.
...
async function uploadDirectory(response, postData) {
    ...
    response.writeHead(200, { 'Content-Type': 'text/html' });
    response.send("Sucessfully Uploaded Files!");
}
...
function sendData(csvFile, imagepath, filename, response) {
    ...
    fs
        ...
        .on('end', () => {
            ...
            const jsonFile = { id: filename, image: imagedata, recipe: "", calories: calories, fat: fat, weight: weight, type: type };
            for (i = 0; i < storeData[0].length; i++) {
                var ingredientName = storeData[1][i].trim();
                jsonFile["recipe"] += " , " + ingredientName;
            }
            nosqldb.insertDocument(jsonFile);
        });
